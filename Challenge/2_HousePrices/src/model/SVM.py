...
class ModelSVM:
    def __init__(self, model_name="svm", task="classification", random_state=42, test_size=0.2):
    ...
    def train(self, df, target_col):
        ...
        if self.task == "classification":
            self.model = SVC(**best_params, random_state=self.random_state, probability=True)
        else:
