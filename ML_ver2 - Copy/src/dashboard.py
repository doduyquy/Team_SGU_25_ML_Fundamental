...
Provides unified dashboard views with multiple visualizations with theme support
"""
...
def _get_themed_layout(title: str, height: int = 600, **kwargs):
    """Helper to get themed layout for dashboard figures"""
    if THEME_AVAILABLE:
        theme = get_theme()
        if theme:
            return theme.get_plotly_layout(title, height=height, **kwargs)
...
def show_overview_dashboard(df, target_col=None, max_features=10):
    """
    Hiển thị dashboard tổng quan cho EDA với theme support
